...
#####
# total
t1_0 <- indicadores %>% filter(mes_crisis_1 == 0) %>% pull(v250_tasa_de_subocupacion) %>% as.double()
t2_0 <- indicadores %>% filter(mes_crisis_2 == 0) %>% pull(v250_tasa_de_subocupacion) %>% as.double()

indicadores <- indicadores %>% 
  mutate(v250_tasa_de_subocupacion = as.double(v250_tasa_de_subocupacion)) %>% 
  mutate(cambio_en_tasa_1 = (v250_tasa_de_subocupacion / t1_0 - 1)*100,
         cambio_en_tasa_2 = (v250_tasa_de_subocupacion / t2_0 - 1)*100)

# hombre
t1_0 <- hombre %>% filter(mes_crisis_1 == 0) %>% pull(v250_tasa_de_subocupacion) %>% as.double()
t2_0 <- hombre %>% filter(mes_crisis_2 == 0) %>% pull(v250_tasa_de_subocupacion) %>% as.double()
...
# mujer
t1_0 <- mujer %>% filter(mes_crisis_1 == 0) %>% pull(v250_tasa_de_subocupacion) %>% as.double()
t2_0 <- mujer %>% filter(mes_crisis_2 == 0) %>% pull(v250_tasa_de_subocupacion) %>% as.double()
